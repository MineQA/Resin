...
- `internal/cloudflare` provides zero-dependency canonical Cloudflare status tokens and helpers (NormalizeSet, IsValidStatus, AllCanonicalStatuses) used across API, service, platform, and probe layers without creating import cycles.
- `internal/config/scoring_policy.go` is a self-contained configuration model for versioned weighted scoring: per-dimension weights (service, API, cloudflare, latency, stability), dimension grade caps, configurable latency/CV bands, CF status scores/grade caps, and grade thresholds.
...
- Node export filtering reuses node list filters: `platform_id`, `subscription_id`, `region`, `egress_ip`, `tag_keyword`, `circuit_open`, `has_outbound`, `enabled`, `routable`, `probed_since`, `quality_profile`, `quality_grade`, `quality_min_score`, `quality_cloudflare_challenged`, `quality_cloudflare_status` (repeated, OR), `quality_checked_since`, `limit`, `offset`. Missing `routable` means no routable-only filter.
- Quality probing uses response-aware fetchers for unconditional target-domain Cloudflare observation, classifying 8 canonical status tokens (`js_challenge`, `captcha_challenge`, `block`, `challenge`, `ng`, `clean`, `not_detected`, `unchecked`) with severity aggregation. The five-dimension scoring engine (service pass rate, API pass rate, Cloudflare status score, latency, stability) applies configurable weights, grade caps, latency/CV bands, and CF-status-specific grade caps. A "balanced" preset provides sensible defaults; legacy scoring is preserved through nil-policy fallback.
- Platform quality filter invariants: `quality_grade` (empty or A/B/C/D/F), `quality_min_score` (0–100), `quality_cloudflare_challenged` (nullable bool), `quality_cloudflare_statuses` (JSON string array, OR match semantics), `quality_checked_since_ns`, `quality_profile`. The API projects the persisted canonical Cloudflare status directly (one of 8 tokens) with legacy empty normalized to `"unchecked"`, not a derived challenged/clean/ng-only subset. The repeated `quality_cloudflare_status` query parameter uses OR semantics; combined with the `quality_cloudflare_challenged` bool filter via AND intersection. Platform JSON-array `quality_cloudflare_statuses` routing filters use identical OR matching.

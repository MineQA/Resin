...
const EXACT_ZH_TO_EN: Record<string, string> = {
  ...
  "更新时间格式无效，请使用 HH:mm": "Invalid update time; use HH:mm",
  "时区不能为空": "Timezone cannot be empty",
  "按所选时区每天执行；服务错过时间后恢复会补执行一次。":
    "Runs daily in the selected timezone; if the service misses the time, it will catch up once after recovery.",
  "每 {{interval}}": "Every {{interval}}",
  "每天 {{time}} · {{timezone}}": "Daily {{time}} · {{timezone}}",

...
func (r *StateRepo) UpsertPlatform(p model.Platform) error {
	...
	protocolFiltersJSON, err := encodeStringSliceJSON(p.ProtocolFilters)
	if err != nil {
		return fmt.Errorf("encode platform %s protocol_filters: %w", p.ID, err)
	}
	excludeProtocolFiltersJSON, err := encodeStringSliceJSON(p.ExcludeProtocolFilters)
	if err != nil {
	...
	_, err = r.db.Exec(`
		...
		ON CONFLICT(id) DO UPDATE SET
			...
			protocol_filters_json    = excluded.protocol_filters_json,
			exclude_protocol_filters_json = excluded.exclude_protocol_filters_json,
...
func (r *StateRepo) GetPlatform(id string) (*model.Platform, error) {
	...
	protocolFilters, err := decodeStringSliceJSON(protocolFiltersJSON)
	if err != nil {
		return nil, fmt.Errorf("decode platform %s protocol_filters_json: %w", p.ID, err)
	}
	excludeProtocolFilters, err := decodeStringSliceJSON(excludeProtocolFiltersJSON)
	if err != nil {
...
func (r *StateRepo) UpsertSubscription(s model.Subscription) error {
	...
	_, err := r.db.Exec(`
			INSERT INTO subscriptions (id, name, source_type, url, content, update_interval_ns,
			                           update_mode, update_time, update_timezone,
			                           enabled,
			                           ephemeral, incremental_alive_nodes, ephemeral_node_evict_delay_ns,
			                           clash_fingerprint_policy, last_checked_ns,
			...
			ON CONFLICT(id) DO UPDATE SET
				...
				update_timezone    = excluded.update_timezone,
				...
				clash_fingerprint_policy     = excluded.clash_fingerprint_policy,
				last_checked_ns    = excluded.last_checked_ns,
				updated_at_ns      = excluded.updated_at_ns
		`, s.ID, s.Name, s.SourceType, s.URL, s.Content, s.UpdateIntervalNs,
		s.UpdateMode, s.UpdateTime, s.UpdateTimezone,
		s.Enabled,
		s.Ephemeral, s.IncrementalAliveNodes, s.EphemeralNodeEvictDelayNs,
		s.ClashFingerprintPolicy, s.LastCheckedNs,
...
func (r *StateRepo) ListSubscriptions() ([]model.Subscription, error) {
	rows, err := r.db.Query(`SELECT id, name, source_type, url, content, update_interval_ns,
		update_mode, update_time, update_timezone,
		enabled,
		ephemeral, incremental_alive_nodes, ephemeral_node_evict_delay_ns,
		clash_fingerprint_policy, last_checked_ns,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.SourceType, &s.URL, &s.Content, &s.UpdateIntervalNs,
			&s.UpdateMode, &s.UpdateTime, &s.UpdateTimezone,
			&s.Enabled,
			&s.Ephemeral, &s.IncrementalAliveNodes, &s.EphemeralNodeEvictDelayNs,
			&s.ClashFingerprintPolicy, &s.LastCheckedNs,
		...
		if s.ClashFingerprintPolicy == "" {
			s.ClashFingerprintPolicy = "reject"
		}
		if s.UpdateMode == "" {

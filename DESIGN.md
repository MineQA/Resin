...
主动探测与被动探测负责基础健康状态（连通性、延迟），但对于节点能否访问特定目标服务（如 AI API）并绕过 Cloudflare 防护，需要独立的质量检查系统。

### 响应感知型 Fetch

主动探测原有的 `Fetcher` 接口（`body, latency, error`）不暴露 HTTP 状态码和响应头，无法判断 Cloudflare 防护状态。质量检查引入 `ResponseFetcher` 接口：

```
type ResponseFetcher func(hash NodeHash, url string) (FetchResponse, error)
```

`FetchResponse` 包含 `Body`、`Latency`、`StatusCode`、`Header`（克隆后的副本）和 `FinalURL`。

当质量检查调用方未提供 `ResponseFetcher` 时，系统自动包装传统 `Fetcher` 为 `LegacyResponseFetcher`（仅暴露 Body，状态码为 0、Header 为 nil）。

### Cloudflare 观测

质量检查**无条件**对所有目标配置文件执行 Cloudflare 观测，不受旧版 `CloudflareDetection` 或 `CloudflareFlag` 字段的门控。观测结果与评分是解耦的两个阶段。

#### 分类流程（证据优先）

1. **cf-mitigated 响应头（权威信号）**：若响应含 `cf-mitigated: challenge`，认定触发挑战；进一步通过 Body 标记判断具体类型；无法区分时归为通用 `challenge`。
2. **状态码提示**：`503` 或 `403` 且存在 Cloudflare 证据（`Server: cloudflare` 或 `CF-Ray` 头）→ `block`。
3. **Body 模式回退**：检查 `__cf_chl_f_tk` / `cf-chl-widget` → `captcha_challenge`；`cf-browser-verification` → `js_challenge`；"just a moment" + "cloudflare" → `js_challenge`；"attention required" + "cloudflare" → `block`；"error code 1020" → `block`。
4. **证据感知输出**：有 Cloudflare 证据但无挑战 → `clean`；无头无状态码的元数据贫乏路径 → `empty/unchecked`；完备响应但无 CF 证据无挑战 → `not_detected`。

#### 自定义 CF 目标 URL

当评分策略（`ScoringPolicy.Cloudflare.TargetURL`）非空且与 `ServiceURL` 不同时，每轮额外发起一次对该 URL 的请求，其观测结果**权威地取代**服务域名的 CF 状态。自定义 CF 目标请求的传输失败直接记为 `ng`。该请求的延迟样本仍然参与延迟维度评分。

### 八种规范状态令牌

系统定义 8 个规范 Cloudflare 状态令牌，在 API、持久化、配置和过滤器之间统一使用：

| 令牌 | 含义 | API 显示 |
|------|------|---------|
| `clean` | 通过 Cloudflare，正常返回服务内容 | `clean` |
| `not_detected` | 目标未通过 Cloudflare（或无 CF 证据） | `not_detected` |
| `js_challenge` | JS 质询（cf-browser-verification） | `js_challenge` |
| `captcha_challenge` | CAPTCHA 质询 | `captcha_challenge` |
| `block` | 被 Cloudflare 拦截（WAF/1020 等） | `block` |
| `challenge` | 通用挑战（cf-mitigated 但无法细分） | `challenge` |
| `ng` | 请求传输失败（DNS/连接/超时） | `ng` |
| `unchecked` | 未检查（默认/遗留空值） | `unchecked` |

`unchecked` 是空/遗留记录的显示形态。持久化空字符串归一化为 `unchecked`。API 查询过滤器可以显式指定 `unchecked` 过滤尚未检查过的节点。

严重度排序（从最严重到最轻）：`block` > `captcha_challenge` > `js_challenge` > `challenge` > `ng` > `clean` > `not_detected` > `empty/unchecked`。多轮聚合取最严重状态。

### 目标配置文件

内置 5 个质量检查目标配置：

| Profile | ServiceURL | APIURL | 指示词 |
|---------|-----------|--------|--------|
| `generic` | `https://www.gstatic.com/generate_204` | 无 | 无 |
| `openai` | `https://chatgpt.com` | `https://api.openai.com/v1/models` | `openai`, `chatgpt` |
| `grok` | `https://grok.com` | `https://api.x.ai/v1/models` | 无 |
| `gemini` | `https://gemini.google.com` | `https://generativelanguage.googleapis.com/v1/models` | `gemini` |
| `claude` | `https://claude.ai` | `https://api.anthropic.com/v1/models` | `claude`, `anthropic` |
...
type ProxyCheckOptions struct {
    ...
    ProtocolDiscovery   bool
    MultiRound          bool
    Rounds              int
    IPInfoEnrichment    bool
...
1. **Service**：服务可达性比率（通过轮数 / 总轮数 × 100）。仅当 `ServiceReachability=true` 且 `Weights.Service > 0` 时参与。
2. **API**：API 可达性比率。仅当 `APIReachability=true`、Profile 有 `APIURL` 且 `Weights.API > 0` 时参与。
3. **Cloudflare**：根据聚合后最严重的 CF 状态查表获得子分（0–100）。`ng` 和 `unchecked` 允许为 nil（即不参与评分）。仅当 `Weights.Cloudflare > 0` 且观测到非空非 `empty` 的状态时参与。
4. **Stability**（稳定性）：取"多轮结果模态一致性"与"延迟 CV 分数"两者中的较低值。一致性低于 100% 时标记 `Unstable=true`。仅当 `MultiRound=true`、轮数 ≥ 2 且 `Weights.Stability > 0` 时参与。
5. **Latency**：全部成功请求的平均延迟，经分档映射得分（0–100）。仅当 `Weights.Latency > 0` 且有延迟样本时参与。
...
| `not_detected` | 100 | 无 |
| `js_challenge` | 40 | D |
| `captcha_challenge` | 20 | D |
| `challenge` | 10 | D |
| `block` | 0 | F |
| `ng`（不可用） | nil | 无 |
| `unchecked`（不可用） | nil | 无 |

> 子分为 nil 表示该状态未参与加权评分（权重视为 0）。等级上限仅在 `Policy` 为 `grade` 或 `score_and_grade` 时应用。
...
| 模式 | 行为 |
|------|------|
| `observe_only` | 仅观测记录 CF 状态，不参与评分和等级上限 |
| `score` | CF 子分参与加权评分 |
| `grade` | CF 触发等级上限（等级上限作用于最终等级） |
| `score_and_grade` | CF 子分参与评分且触发等级上限 |

### Balanced 预设

Balanced 是系统默认的评分策略预设，在未配置规范评分策略或从旧版字段派生时使用：
...
```json
{
  ...
  "cloudflare": {
    ...
    "target_url": "",
    "status_scores": { "clean": 100, "not_detected": 100, "js_challenge": 40, "captcha_challenge": 20, "challenge": 10, "block": 0, "ng": null, "unchecked": null },
    "grade_caps": { "js_challenge": "D", "captcha_challenge": "D", "challenge": "D", "block": "F" }
  },
  "latency": {
    "bands": [
...
加权总分经四舍五入（half-away-from-zero）后 clamped 到 0–100。最终等级由分数阈值决定，再叠加所有已应用的等级上限（取最严格者）。

### 维度等级上限（可选）

每个维度可配置独立的 `BelowScore` + `GradeCap` 规则：当该维度子分低于 `BelowScore` 时，最终等级上限为 `GradeCap`。Balanced 预设所有维度上限均为 nil（不启用）。等级上限可叠加，取最严格者生效。

### 兼容性

规范评分策略（`ProxyCheckScoring`，`ScoringPolicy` 嵌套对象）与旧版扁平字段（`ProxyCheckServiceReachability`、`ProxyCheckAPIReachability`、`ProxyCheckCloudflareDetection`、`ProxyCheckMultiRound`）共存。

- 当 `ProxyCheckScoring` 非空且 `Version ≥ 1` 时，完全使用规范策略，忽略旧版扁平字段。
- 当 `ProxyCheckScoring` 为 nil 或 `Version < 1` 时，从旧版扁平字段构建兼容策略：基于 Balanced 预设，将对应维度权重清零（`CloudflareDetection=false` 时将 CF 权重归零且策略设为 `observe_only`；`MultiRound=false` 时将稳定性权重归零；Latency 始终参与）。
- 旧版 `CloudflareChallenged` 和 `CloudflareChallengeType` 字段在新版路径中从聚合 `CloudflareStatus` 派生以保持向后兼容。

### 评分结果结构

`ScoringResult` 包含完整评分信息：
...
```json
{
  ...
  "grade_from_score": "A",
  "final_grade": "A",
  "effective_weights": { "service": 40, "cloudflare": 20, "latency": 15 },
  "sub_scores": {
    "service": { "value": 100 },
    "api": { "unavailable": true },
    "cloudflare": { "value": 100 },
    "stability": { "unavailable": true },
    "latency": { "value": 80 }
  },
  "unavailable_dimensions": ["api", "stability"],
  "applied_caps": [],
  "terminal_reason": ""
}
```

其中 `ScoreBreakdown` 以紧凑 JSON 形式持久化到 `node_quality.score_breakdown` 列中，API 返回时作为 `json.RawMessage` 直接透传。
...
* node_quality(node_hash, profile, grade, score, unstable, service_reachable, api_reachable, cloudflare_challenged, cloudflare_challenge_type, avg_latency_ms, last_checked_ns, last_error, cloudflare_status TEXT, scoring_policy_version INTEGER, score_breakdown TEXT, PK(node_hash,profile))。`cloudflare_status` 存储规范 CF 状态令牌（空字符串表示遗留/未更新）；`scoring_policy_version` 记录产生此结果的评分策略版本（0=旧版）；`score_breakdown` 存储紧凑 JSON 的评分分解。
...
* `proxy_check_interval` 必须是合法 duration。
* `proxy_check_profile` 必须为已知 Profile 名称（generic/openai/grok/gemini/claude）。
* `proxy_check_scoring` 可为 null 或合法 ScoringPolicy 对象（version=1，权重合规，CF 策略合规等）。
...
* 必填字段：`name`
* 可选字段：`sticky_ttl`、`regex_filters`、`region_filters`、`protocol_filters`、`exclude_protocol_filters`、`reverse_proxy_miss_action`、`reverse_proxy_empty_account_behavior`、`reverse_proxy_fixed_account_header`、`allocation_policy`、`passive_circuit_breaker_disabled`、`quality_grade`、`quality_min_score`、`quality_cloudflare_challenged`、`quality_cloudflare_statuses`、`quality_checked_since_ns`、`quality_profile`
* 不可传字段：`id`、`updated_at`、`routable_node_count`
* 省略可选字段时，平台策略字段使用当前环境变量默认平台设置（`RESIN_DEFAULT_PLATFORM_*`）对应值；`passive_circuit_breaker_disabled` 默认 `false`；质量筛选字段使用空默认值（不启用筛选）
...
* `quality_grade`：空字符串或 A/B/C/D/F。指定后，预览过滤器只返回质量等级匹配的节点。
* `quality_cloudflare_challenged`：可空布尔。`true` 只返回被 CF 拦截的节点，`false` 只返回未被拦截的节点，`null` 不筛选。
* `quality_cloudflare_statuses`：规范状态令牌数组（可选值：`clean`、`not_detected`、`js_challenge`、`captcha_challenge`、`block`、`challenge`、`ng`、`unchecked`）。元素去重并按规范顺序排序后存储，使用 OR 语义匹配。空数组不筛选。非法令牌返回 400。
* `quality_checked_since_ns`：纳秒时间戳，仅返回在该时间之后进行过质量检查的节点。
* `quality_profile`：按目标配置文件名称筛选（如 `openai`、`generic`）。

> **注意**：启用任意质量过滤器后，未完成质量检查的节点将被自动排除。新加入系统的节点在首次质量检查完成之前均无质量数据，因此启用质量过滤器可能导致可路由节点数为零，直到质量检查服务完成至少一轮检查。
...
```json
{
  "platform_spec": {
    ...
    "quality_cloudflare_statuses": ["clean", "not_detected"]
  }
}
```
...
* `platform_spec` 仅允许字段：`regex_filters`、`region_filters`、`protocol_filters`、`exclude_protocol_filters`、`quality_grade`、`quality_min_score`、`quality_cloudflare_challenged`、`quality_cloudflare_statuses`、`quality_checked_since_ns`、`quality_profile`。
...
* `platform_spec.quality_cloudflare_statuses`：数组元素必须为规范状态令牌（如 `clean`、`block`），空令牌或未知令牌返回 400。
...
* `quality_grade`：A/B/C/D/F（可选），按节点质量检查等级过滤。缺失质量检查数据的节点不匹配。
* `quality_min_score`：0–100（可选），要求节点质量评分 ≥ 该值。缺失质量数据的节点不匹配。
* `quality_cloudflare_challenged`：true|false（可选），按节点质量检查的 CF 拦截状态过滤。缺失质量数据的节点不匹配。
* `quality_cloudflare_status`：可重复（可选），如 `?quality_cloudflare_status=clean&quality_cloudflare_status=block`。在同一参数内使用 OR 语义——匹配任一指定状态的节点均符合。空持久化状态归一化为 `unchecked` 参与匹配。缺失质量数据的节点不匹配。与 `quality_cloudflare_challenged` 等其他质量过滤器使用 AND 语义。
* `quality_checked_since`：RFC3339Nano（可选），要求节点质量检查时间 ≥ 该值。缺失质量数据的节点不匹配。
* `quality_profile`：profile 名称（可选），按质量检查使用的配置文件过滤。缺失质量数据的节点不匹配。

> **注意**：启用任意质量过滤器（`quality_grade`、`quality_min_score`、`quality_cloudflare_challenged`、`quality_cloudflare_status`、`quality_checked_since`、`quality_profile`）后，未完成质量检查的节点将被自动排除。新加入系统的节点在首次质量检查完成之前均无质量数据，因此启用质量过滤器可能导致可路由节点数为零，直到质量检查服务完成至少一轮检查。

* `sort_by`：排序字段（可选）
* `sort_order`：`asc` 或 `desc`（可选）

支持的 `sort_by`：`tag`、`created_at`、`failure_count`、`region`、`quality_score`、`quality_last_checked`。其中 `tag` 表示按照节点 tag 排序。如果有多个 tag，按照所属 subscription 创建最早的那个。同 subscription 选字典序最小的。默认按 `tag` `asc` 排序。`quality_score` 排序时，缺失质量数据的节点按 -1 分参与排序（升序排列在最前，降序排列在最后）。`quality_last_checked` 排序时，缺失质量数据的节点按 0 处理。
  ...
  "items": [
    {
      ...
      "quality": {
        "quality_profile": "openai",
        "quality_grade": "A",
        "quality_score": 96,
        "quality_unstable": false,
        "quality_service_reachable": true,
        "quality_api_reachable": false,
        "quality_cloudflare_challenged": false,
        "quality_cloudflare_challenge_type": "",
        "quality_avg_latency_ms": 450.0,
        "quality_last_checked": "2026-02-10T12:30:00Z",
        "quality_last_error": "",
        "quality_cloudflare_status": "clean",
        "quality_scoring_policy_version": 1,
...
* 过滤参数同“列出节点”：`platform_id`、`subscription_id`、`region`、`egress_ip`、`tag_keyword`、`circuit_open`、`has_outbound`、`enabled`、`routable`、`probed_since`、`quality_grade`、`quality_min_score`、`quality_cloudflare_challenged`、`quality_cloudflare_status`（可重复，OR）、`quality_checked_since`、`quality_profile`、`limit`、`offset`。
...
#### 代理质量检查设置
* `ProxyCheckEnabled`: 是否启用代理质量检查（主动探测节点目标服务可达性与 Cloudflare 状态）。默认 false。
* `ProxyCheckInterval`: 两次质量检查之间的最小间隔。默认 30 分钟。
* `ProxyCheckProfile`: 目标配置文件名称。默认 `generic`。可选值：`generic`、`openai`、`grok`、`gemini`、`claude`。
* `ProxyCheckServiceReachability`: 检查服务可达性。默认 true。
* `ProxyCheckAPIReachability`: 检查 API 可达性。默认 false。
* `ProxyCheckCloudflareDetection`: 旧版兼容字段。当规范评分策略未配置时，`true` 表示 CF 子分参与评分和等级上限，`false` 表示仅观测（`observe_only`）。将来会移除。默认 true。
* `ProxyCheckMultiRound`: 是否启用多轮检查。启用后检查轮数由 `ProxyCheckRounds` 控制。默认 false。
* `ProxyCheckRounds`: 多轮检查的轮数。仅 `ProxyCheckMultiRound=true` 时生效。默认 1。
* `ProxyCheckTriggerOnNewNode`: 新节点加入时是否立即触发质量检查。默认 false。
* `ProxyCheckScoring`: 规范评分策略嵌套对象。当非空且 `version>=1` 时，完全使用此策略评分，忽略上述扁平兼容字段。结构见上文的 Balanced 预设。
